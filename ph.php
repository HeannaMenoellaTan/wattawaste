...
:root {
    ...
    --ok: #22c55e; --warn: #f59e0b; --acidic: #ef4444; --alkaline: #3b82f6;
}
...
.history-item {
    ...
    border-left: 4px solid #eab308; transition: all 0.2s;
}
.history-item:hover { background: #f1f5f9; transform: translateX(4px); }
.history-value { font-size: 22px; font-weight: 700; color: #ca8a04; }
...
.recommendation-popup {
    ...
    z-index: 1000; max-width: 350px; border-left: 5px solid;
...
function updatePHIndicator(ph) {
    const pct = Math.min(Math.max((ph / 14) * 100, 0), 100);
    document.getElementById('phIndicator').style.left = `calc(${pct}% - 10px)`;
}

function getPHStatus(ph) {
    if (ph < 6.0) return {
        status: 'Too Acidic',      statusClass: 'acidic',
        desc: 'Add brown materials or lime to increase pH.',
        recommendation: 'Add brown/lime materials to neutralize acidity'
    };
    if (ph > 8.0) return {
        status: 'Too Alkaline',    statusClass: 'alkaline',
    ...
    return {
        status: 'Optimal (Healthy)', statusClass: 'ok',
...
function updatePHDisplay(ph) {
    ...
    hero.classList.add(info.statusClass);
    badge.classList.add(info.statusClass);
    popup.classList.add(info.statusClass);

    if (ph < 6.0 || ph > 8.0) {
        banner.classList.add(info.statusClass);
        banner.style.display = 'block';
        document.getElementById('warningText').textContent        = info.recommendation;
...
function renderChart(entries) {
    ...
    phChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: false, min: 4, max: 10,
...
function renderHistory(entries) {
    ...
    recent.forEach(entry => {
        ...
        if      (val < 6.0) { badgeClass = 'danger';  badgeText = 'Too Acidic';   }
        else if (val > 8.0) { badgeClass = 'primary'; badgeText = 'Too Alkaline'; }
...
function loadHistoryData(range) {
    ...
    const histRef = window.firebaseQuery(
        window.firebaseRef(db, 'sensors/ph/history'),
...
function showNoData() {
    ...
    ['maxPHStat','avgPHStat','minPHStat'].forEach(id =>
...
window.initializePHMonitoring = function () {
    ...
    // Live latest pH — real-time listener, no throttle needed
    const phRef = window.firebaseRef(db, 'sensors/ph/latest');
