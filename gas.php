...
.history-item {
    ...
    border-left: 4px solid #ffba08; transition: all 0.2s;
}
...
function getGasStatus(gas) {
    if (gas >= 800) return {
        status: 'Critical (Mixer Activated)', statusClass: 'crit',
        desc: 'Gas level extremely high. Automatic mixer activation.'
    };
    if (gas >= 600) return {
        status: 'Warning (High)',             statusClass: 'warn',
    ...
    return {
        status: 'Below Range',                statusClass: 'info',
...
function updateGasDisplay(gas) {
    ...
    if      (gas >= 800) { critBanner.style.display = 'block'; warnBanner.style.display = 'none';  }
    else if (gas >= 600) { critBanner.style.display = 'none';  warnBanner.style.display = 'block'; }
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: (ctx)   => ' Gas Level: ' + ctx.parsed.y.toFixed(0) + ' ppm'
                    }
                }
            },
            scales: {
                y: {
                    beginAtZero: true, min: 0, max: 1000,
...
function renderHistory(entries) {
    ...
    recent.forEach(entry => {
        ...
        if      (val >= 800) { badgeClass = 'danger';          badgeText = 'Critical';     }
        else if (val >= 600) { badgeClass = 'warning text-dark'; badgeText = 'Warning';    }
        else if (val >= 200) { badgeClass = 'success';           badgeText = 'Optimal';    }
        else                 { badgeClass = 'info';              badgeText = 'Below Range'; }
...
function loadHistoryData(range) {
    ...
    const histRef = window.firebaseQuery(
        window.firebaseRef(db, 'sensors/gas/history'),
...
function showNoData() {
    ...
    ['maxGasStat','avgGasStat','minGasStat'].forEach(id =>
        document.getElementById(id).textContent = '-- ppm');
...
window.initializeGasMonitoring = function () {
    ...
    // Live latest gas — real-time listener, no throttle needed
    const gasRef = window.firebaseRef(db, 'sensors/gas/latest');
