...
try {
    ...
    error_log("Latest snapshot exists: " . ($latestSnapshot->exists() ? 'YES' : 'NO'));
    
    if ($latestSnapshot->exists()) {
        $latestData = $latestSnapshot->getValue();
        error_log("Latest data: " . print_r($latestData, true));
        
        $currentWeight = $latestData['value'] ?? 0;
        $weightCapacity = $latestData['capacity'] ?? 25; // Default 25kg capacity
        
        error_log("Current Weight: $currentWeight, Capacity: $weightCapacity");
    } else {
        error_log("No latest weight data found in Firebase");
        $currentWeight = 0;
        $weightCapacity = 25;
    }
...
<div class="history-section">
    ...
    <table class="history-table">
        ...
        <?php foreach ($historyData as $index => $entry): 
            // Highlight specific rows if needed (e.g., significant weight changes)
            $highlightClass = '';
        ?>
